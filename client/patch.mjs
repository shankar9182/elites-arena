...
const sidebarCode = `                        <div className="px-4 py-2 mt-4 mb-2">
                             <button
...
// 1. Replace Bell
content = content.replace(/<div className="relative cursor-pointer group">[\s\S]*?<Bell size=\{18\} className="text-\[var\(--text-secondary\)\] group-hover:text-\[var\(--text-primary\)\]" \/>[\s\S]*?<\/div>[\s\S]*?<span[\s\S]*?<\/span>[\s\S]*?<\/div>/, newBellBlock);

// 2. Replace Sidebar
content = content.replace(/<\/nav>/, sidebarCode); // Replace the first closing nav (there should only be one main nav)

// 3. Replace Footer
content = content.replace(/<\/div>\s*<\/div>\s*\);\s*};/, modalCode);
